...
PIXI.Graphics.prototype.bezierCurveTo2 = function(
  cpX,
  cpY,
  cpX2,
  cpY2,
  ...
  for (var i = 1; i <= n; i++) {
    ...
    let nPx = dt3 * fromX + 3 * dt2 * j * cpX + 3 * dt * t2 * cpX2 + t3 * toX;
    let nPy = dt3 * fromY + 3 * dt2 * j * cpY + 3 * dt * t2 * cpY2 + t3 * toY;
    ...
    var distance = Math.abs(
      Math.sqrt(Math.pow(nPx - lPx, 2) + Math.pow(nPy - lPy, 2))
    );
    if (state === 0) {
...
PIXI.Graphics.prototype.quadraticCurveTo2 = function(cpX, cpY, toX, toY) {
  ...
  for (var i = 1; i <= n; i++) {
    ...
    let nPx = xa + (cpX + (toX - cpX) * j - xa) * j;
    let nPy = ya + (cpY + (toY - cpY) * j - ya) * j;
  ...
  if (this._nonScaling) {
    for (var i = 0; i < this.graphicsData.length; i++) {
      if (this.graphicsData[i].lineWidth > 0) {
        this.graphicsData[i].lineWidth =
          this.graphicsData[i].lineWidth / this.scale.x;
      }
    }
  }
};
...
SVGGraphics.prototype.drawNode = function(parent, node) {
  ...
  if (typeof this["draw" + capitalizedTagName + "Node"] !== "undefined") {
    if (capitalizedTagName === "Svg") {
      ...
      graphics
        .beginFill(0x000, 0)
        .drawRect(0, 0, parseInt(width), parseInt(height));
    }
...
SVGGraphics.prototype.drawTextNode = function(parent, node) {
  ...
  pixi_text.y =
    parseInt(y) -
    pixi_text.height +
    PIXI.TextMetrics._fonts[pixi_text._font]
      .descent(parent || this)
      .addChild(pixi_text);
};
...
SVGGraphics.prototype.drawPolylineNode = function(graphics, node) {
  ...
  //var reg = "(-?[\\d\\.?]+),(-?[\\d\\.?]+)";
  var points = node.getAttribute("points").split(" ");//.match(new RegExp(reg, "g"));
  ...
  for (var i = 0; i < instructions.length; i++) {
    ...
    while (z < points.length) {
      switch (command.toLowerCase()) {
        ...
        case "Z":
        case "z":
          graphics.currentPath.closeStroke = true;
          z += 1;
          break;
        default:
...
SVGGraphics.prototype.tokenizePathData = function(pathData) {
  var commands = pathData.match(/[a-df-z][^a-df-z]*/gi);
  var data = {
    instructions: []
  };
...
SVGGraphics.prototype.getInheritedStyleAttribute = function(
  graphics,
  attributeName
) {
  if (graphics._styleAttributes && graphics._styleAttributes[attributeName]) {
    ...
    const value = this.getInheritedStyleAttribute(
      graphics.parent,
      attributeName
    );
    if (value) {
...
SVGGraphics.prototype.applySvgAttributes = function(graphics, attributes) {
  ...
  styleAttributeNames.forEach(attributeName => {
    styleAttributes[attributeName] =
      ...
      this.getInheritedStyleAttribute(graphics, attributeName);
  });
  ...
  if (
    ...
    styleAttributes["stroke-dasharray"] !== "none"
  ) {
